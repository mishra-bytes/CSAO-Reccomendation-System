...
The CSAO business impact model projects **₹13–70 Cr incremental GMV annually**
(10th–90th percentile over key assumptions), with a point estimate of ~₹37 Cr.
At Zomato's ~22% take rate, this translates to **₹3–15 Cr Zomato revenue**.
...
```
NDCG@10 = 0.707  (offline eval, temporal split)
      ↓
baseline_NDCG = 0.614  (Popularity baseline)
      ↓
ndcg_improvement = 0.707 - 0.614 = 0.093
      ↓
relative_ctr_uplift = 0.5 × (0.093 / 0.614) = 7.6%
      ↓
baseline_ctr = 8% → new_ctr = 8% × 1.076 = 8.61%
      ↓
ctr_delta = 0.61 pp × click_to_cart(0.50) = 0.30 pp effective
      ↓
incremental_aov = 0.0030 × avg_addon_price(₹128) = ₹0.39/order
      ↓
daily_exposed_orders = 2.5M DAU × 1.3 orders × 80% exposure = 2.6M
      ↓
daily_incremental_gmv = ₹0.39 × 2.6M = ₹1.01M
      ↓
annual_incremental_gmv ≈ ₹37 Cr
annual_zomato_revenue ≈ ₹8 Cr (at 22% take rate)
```
...
|---|-----------|-------|--------|------------|-------------|
| 1 | Baseline NDCG | 0.614 | Popularity baseline from offline eval | **High** — from our eval | Low |
| 2 | CSAO NDCG | 0.707 | Offline eval, temporal split, leakage-free | **High** | Medium |
...
|----------|------------|-------------|----------|-----------------|
| Very Conservative | 0.25 | 4% | 60% | **₹7** |
| Conservative | 0.25 | 8% | 80% | **₹14** |
| Central | 0.50 | 8% | 80% | **₹37** |
| Moderate | 0.50 | 12% | 80% | **₹42** |
| Optimistic | 0.75 | 12% | 95% | **₹99** |
...
| Sensitivity analysis | Disconnected (hardcoded 0.03 base) | Uses actual NDCG delta (0.093) with calibration/CTR/exposure grid |
...
1. **Per-order impact is modest**: ₹0.2–0.6/order incremental AOV — an Egg Samosa
   costs ₹30, so the model implies ~0.5–2% of exposed users add one item. Believable.
...
4. **A/B test plan is well-designed**: Power analysis correctly sized — detects
   ₹0.39 AOV lift with 80% power in ~2 weeks. This IS the primary validation.
...
> "We project ₹13–70 Cr incremental GMV (₹3–15 Cr Zomato revenue) from CSAO
> recommendations, driven by a 7.6% relative CTR uplift from NDCG improvement.
> The per-order impact is ₹0.2–0.6 — a single beverage or side dish. The headline
> number is driven by Zomato's 2.6M daily order scale. **All projections are
